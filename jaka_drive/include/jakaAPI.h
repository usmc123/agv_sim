...
//#endif // __cpluscplus
	...
	* @return ERR_SUCC 成功 其他失败
	*/
	DLLEXPORT_API errno_t torque_control_enable(const JKHD *handle, BOOL enable);
	...
	* @return ERR_SUCC 成功 其他失败
	* @description: 需要确认不超过当前力矩的30%, 当前瞬时力矩可通过机器人状态参数获取
	*/
	DLLEXPORT_API errno_t torque_feedforward(const JKHD *handle, TorqueValue tor_val, int grv_flag);
